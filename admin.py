...
from utils.db import mycur, mydb
...
print()
console.print(
    "[red]LIPS Library Management System[/red]",
    justify="center",
)
...
if ch == "1":
    while True:
        ...
        while True:
            ...
            if (
                mycur.execute(
                    f"SELECT * FROM BOOKS WHERE ID = '{id}' AND ISSUED_TO = 0"
                ).fetchone()
                != None
            ):
                print_book(binfo)
                ...
                if chch.lower() not in ("n", "nn", "not"):
                    ...
                    mycur.execute(
                        f"UPDATE BOOKS SET ISSUED_TO = {sid}, ISSUE_DATE = '{today}', RESERVED = 0 WHERE ID = {id}"
                    )
                    mydb.commit()
        ...
        if mycur.execute(f"SELECT * FROM BOOKS WHERE ID = '{id}'").fetchone()[4] != 0:
            ...
            if chch.lower() not in ("n", "nn", "not"):
                ...
                if days >= 15:
                    ...
                    fine = 0
                mycur.execute(
                    f"UPDATE BOOKS SET ISSUED_TO = 0 , ISSUE_DATE = NULL WHERE ID = '{id}'"
                )
                mydb.commit()
                mycur.execute(
                    f"UPDATE ACCOUNTS SET AMMOUNT = AMMOUNT + {fine} WHERE ID = {binfo[4]}"
                )
                mydb.commit()
                sinfo = mycur.execute(
                    f"SELECT * FROM ACCOUNTS WHERE ID = {binfo[4]}"
                ).fetchone()
    ...
    while True:
        ...
        mycur.execute(
            f"INSERT INTO BOOKS (ID,TITLE,AUTHOR,PRICE) VALUES ({id}, '{title}','{author}',{price})"
        )
        mydb.commit()
        print("[green]Book Added Successfully! ")
        time.sleep(2)
        if input("Do You Want to Add More ? [red](y/n)[/]").lower() in (
            "n",
            "nn",
            "not",
        ):
    ...
    while True:
        ...
        if chch in ("1", "2", "3", "4"):
            ...
            time.sleep(2)
            if input("Do You want to edit more? [red](y/n)[/]").lower() in (
                "n",
                "nn",
                "not",
            ):
    ...
    while True:
        ...
        if (
            input(
                '[red bold]Are you sure you want to delete this book? type [green]"yes"[/] >> [/]'
            ).lower()
            == "yes"
        ):
