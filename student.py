    ...
    if binfo[6] == 0:
        if binfo[4] != 0:
            ...
            if (
                len(
                    mycur.execute(
                        f"SELECT * FROM BOOKS WHERE RESERVED = {data[0]}"
                    ).fetchall()
                )
                > 2
            ):
                ...
                if input("Do You Want to Reserve This Book? (y/n)") not in (
                    "n",
                    "nn",
                    "not",
                ):
                    mycur.execute(
                        f"UPDATE BOOKS SET RESERVED = {data[0]} WHERE ID = {binfo[0]}"
                    )
                    mydb.commit()
    ...
    if check_internet():
        ...
        while True:
            if input("Enter OTP > ") == str(code):
                mycur.execute(
                    f"UPDATE ACCOUNTS SET EMAIL = '{emailid}' WHERE ID = {data[0]}"
                )
                mydb.commit()
    ...
    if data[4] > 0:
        ...
        if chch == "1":
            ...
            if check_internet():
                ...
                data2 = generate_payment(
                    input("Enter Mobile Number>>"), linkid, ammount, note
                )
                mycur.execute(
                    f"""UPDATE ACCOUNTS SET LINKID = '{data2["link_id"]}' WHERE ID = {data[0]}"""
                )
                mydb.commit()
                print("Sending Mail...")
            ...
            linkid = mycur.execute(
                f"SELECT * FROM ACCOUNTS WHERE ID = {data[0]}"
            ).fetchone()[6]
            if check_internet():
                if verify_payment(linkid)["link_status"] == "PAID":
                    mycur.execute(
                        f"UPDATE ACCOUNTS SET AMMOUNT = 0, LINKID = Null WHERE ID = '{data[0]}'"
                    )
                    mydb.commit()
                else:
