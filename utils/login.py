    ...
    if role == "1":
        ...
        if ch == "1":
            while True:
                ...
                ucheck = mycur.execute(
                    f"SELECT * FROM ACCOUNTS WHERE USERNAME = '{uname}'"
                )
            ...
            while True:
                if int(input("Enter Your Code >> ")) == code:
                    mycur.execute(
                        f"INSERT INTO ACCOUNTS (TYPE,USERNAME,PASSWORD,EMAIL) VALUES('S','{uname}','{password}','{eml}')"
                    )
                    mydb.commit()
        ...
        if ch == "2":
            ...
            while True:
                password = hashlib.md5(
                    input("Enter Your password : ").encode()
                ).hexdigest()
                mycur.execute(
                    f"SELECT * FROM ACCOUNTS WHERE USERNAME = '{uname}' AND PASSWORD = '{password}'"
                )
                ldta = mycur.fetchone()
                if ldta != None:
        ...
        if ch == "3":
            ...
            while True:
                if int(input("Enter Your Code >> ")) == code:
                    password = hashlib.md5(
                        input("Enter Your New password : ").encode()
                    ).hexdigest()
                    mycur.execute(
                        f"UPDATE ACCOUNTS SET PASSWORD = '{password}' WHERE USERNAME = '{uname}'"
                    )
                    mydb.commit()
